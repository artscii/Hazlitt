...
assert.equal((await call('/api/records','POST',{})).status,401);assert.equal((await call('/api/audit')).status,401);assert.equal((await call('/api/login','POST',{password:'wrong'})).status,401);assert.equal((await call('/api/login','POST',{password:'Bombo'})).status,200);
...
let logs=(await call('/api/audit')).data.entries;assert.equal(logs.length,2);const edit=logs.find(l=>l.action==='update');assert.equal(edit.ip,'203.0.113.8');assert.equal(edit.revision,2);
...
const create=logs.find(l=>l.action==='create');assert.equal((await call('/api/rollback/'+create.id,'POST',{side:'before',revision:5})).status,200);assert(!(await call('/api/catalog')).data.programs.some(p=>p.id===id));
assert.equal((await call('/api/logout','POST',{})).status,200);assert.equal((await call('/api/audit')).status,401);
console.log('PASS: authentication, protected audit access, CRUD, duplicate names, CSRF, URL validation, stale revisions, edit IPs, rollback, deletion restoration and logout.');
