...
assert.equal((await call('/api/analytics/event','POST',{visit,project:'bombo'})).data.ignored,true);
assert.equal(sqlite.prepare('SELECT COUNT(*) n FROM analytics_visits').get().n,0);
...
env.UMAMI_WEBSITE_ID=crypto.randomUUID();env.UMAMI_DASHBOARD_URL='https://atlas.test:8443/';
let config=(await call('/api/analytics/config')).data;assert.equal(config.enabled,true);assert.equal(config.dashboardUrl,'');
env.UMAMI_DASHBOARD_URL='javascript:alert(1)';assert.equal((await call('/api/analytics/config')).data.enabled,false);
env.UMAMI_DASHBOARD_URL='https://atlas.test:8443/';
assert.equal((await call('/api/login','POST',{password:'Bombo'})).status,200);
config=(await call('/api/analytics/config')).data;assert.equal(config.enabled,false);assert.equal(config.dashboardUrl,env.UMAMI_DASHBOARD_URL);
console.log('PASS analytics retirement, no legacy writes, origin protection, Umami configuration and authenticated visitor exclusion');
