...
const input=w.document.querySelector('input');const run=q=>{input.value=q;return w.atlasPrimarySearch(q,()=>fallbacks++);};
...
await run('other');assert.equal(fallbacks,2);await run('');assert.equal(fallbacks,3);
