...
if (!secret || secret.length < 32)
  throw Error("QMD_SERVICE_TOKEN must contain at least 32 characters");
...
async function getCatalog() {
  ...
  if (response.status === 304 && catalog) {
    expires = Date.now() + 30000;
    return catalog;
  }
  if (!response.ok) throw Error("Catalogue unavailable");
  const data = await response.json();
  if (!Array.isArray(data.programs)) throw Error("Invalid catalogue");
  etag = response.headers.get("etag");
  catalog = { ...data, revision: etag };
  expires = Date.now() + 30000;
  return catalog;
}
let tokens = 12,
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/api/search/status" && req.method === "GET")
  ...
  try {
    ...
    const request = new Request("http://qmd/api/search/query", {
      method: "POST",
      signal: abort.signal,
      headers: { Origin: "http://qmd", "Content-Type": "application/json" },
    ...
    console.log(
      "QMD_REQUEST " +
        JSON.stringify({
          ...
          cpuMs: Math.round((usage.user + usage.system) / 1000),
    ...
    reply(503, { error: "Semantic search unavailable" });
  }
});
