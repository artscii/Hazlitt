...
const publicFields = AtlasSearch.publicFields;
    ...
    publicFields
      ...
      .map(
        (k) =>
          ...
          (Array.isArray(p[k])
            ? p[k].join(", ")
            : String(p[k]).replace(/<[^>]*>/g, " ")),
      )
      .join("\n\n")
  );
}
export function eligible(p, query, scope = {}) {
  const parsed = AtlasSearch.parse(query);
  return (
    (!scope.country ||
      p.countries?.some(
        (c) => c.toLowerCase() === scope.country.toLowerCase(),
      )) &&
    (!scope.basis ||
      p.evidenceBasis?.toLowerCase() === scope.basis.toLowerCase()) &&
    (!scope.year || Number(p.publicationYear) === Number(scope.year)) &&
    (!scope.continent ||
      AtlasSearch.continentsOf(p).some(
        (c) => c.toLowerCase() === scope.continent.toLowerCase(),
      )) &&
    parsed.countries.every((c) => p.countries?.includes(c)) &&
    parsed.continents.every((c) =>
      AtlasSearch.continentsOf(p).some((x) => x.toLowerCase() === c),
    ) &&
    parsed.years.every(
      ([a, b]) =>
        Number(p.publicationYear) >= a && Number(p.publicationYear) <= b,
    )
  );
}
export function createSearchService({
  ...
  async function sync(programs) {
    ...
    const files = new Set();
    for (const d of docs) {
      const name = createHash("sha256").update(d.id).digest("hex") + ".md";
      files.add(name);
      ...
      if (old !== d.text) await fs.writeFile(file, d.text);
    }
    ...
    if (!store) {
      const createStore =
        openStore || (await import("@tobilu/qmd")).createStore;
      store = await createStore({
        dbPath: path.join(root, "index.sqlite"),
        config: { collections: { atlas: { path: dir, pattern: "**/*.md" } } },
      });
    }
    ...
    await store.update();
    await store.embed({ collection: "atlas" });
    version = digest;
  ...
  async function searchRoute(request, getCatalog) {
    ...
    if (
      ...
      return reply({ error: "Enter a query of 1–500 characters." }, 400);
    const started = performance.now();
    if (!snapshot) {
      readiness(getCatalog);
      return reply({ error: "Search warming up" }, 503);
    }
    let parsed;
    try {
      parsed = parseParameters(input.query, input.scope || {});
    } catch (e) {
    ...
    const active = snapshot,
      allowed = new Set(
        active.programs
          .filter((p) => eligible(p, parsed.query, parsed.scope))
          .map((p) => p.id),
      );
    ...
    if (
      ...
      active.programs.some(
        (p) =>
          AtlasSearch.normalize(p.name) === AtlasSearch.normalize(parsed.query),
      )
    )
      return respond(base, { lexicalOnly: true });
    ...
    try {
      const results = await queue.run(
        ...
        async () => {
          ...
          const timings = {},
            ...
            t = performance.now();
          try {
            const vector = await store.searchVector(parsed.query, {
              collection: "atlas",
              limit: active.programs.length,
            });
            const byFile = new Map(
              vector.map((r) => [path.basename(r.file || r.filepath), r.score]),
            );
            // Vector rank is used only for candidates supported by approved related concepts.
            const related = candidates
              .filter((r) => byFile.has(active.files.get(r.id)))
              .sort(
                (a, b) =>
                  byFile.get(active.files.get(b.id)) -
                  byFile.get(active.files.get(a.id)),
              );
            const results = [...base, ...related];
            cache.set(key, results);
            if (cache.size > 80) cache.delete(cache.keys().next().value);
            return results;
          } finally {
            Object.assign(timings, profile.finish(), {
              queryMs: performance.now() - t,
            });
            console.log("QMD_PROFILE " + JSON.stringify(timings));
          }
        },
        request.signal,
      );
      ...
      lastError = e.message;
      if (!/cancelled|busy|expired/.test(e.message)) ready = false;
      return respond(base, { lexicalOnly: true, semanticUnavailable: true });
    }
  }
        ...
        try {
          ...
          snapshot = {
            ...
            files: new Map(
              programs.map((p) => [
                p.id,
                createHash("sha256").update(p.id).digest("hex") + ".md",
              ]),
            ),
          };
          cache.clear();
          ...
          lastError = e.message;
          ready = false;
          console.error("QMD background refresh:", e.message);
        } finally {
          profile?.finish();
          refreshAt = Date.now();
          refreshing = false;
