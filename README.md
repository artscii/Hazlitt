...
### System diagrams and live Docker status (4.13.25)
...
Shares require the public server and its persistent SQLite volume. Database backups include them. Creation is bounded to 10,000 distinct links; existing links remain readable at that limit. As of 4.13.34, the URL stays hidden: clipboard denial shows a short message asking the visitor to allow clipboard access and retry. Successful copying shows “(Link copied)” until the pointer or focus leaves the paperclip.
