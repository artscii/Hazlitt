...
globalThis.AtlasWorkbook=(()=>{
 ...
 function makeDiffWorkbook(ExcelJS,preview){
  ...
  summary.addRows([['Source file',preview.filename],['Preview generated',new Date(preview.at).toISOString()],['Review only','This workbook does not apply changes. Return to Admin, preview again if the source file changes, and confirm the selected bulk import.'],['Rows',preview.total],['Selected additions',preview.rows.filter(r=>r.status==='new'&&selected.has(r.key)).length],['Selected updates',preview.rows.filter(r=>r.status==='update'&&selected.has(r.key)).length],['Skipped rows',preview.total-selected.size],['Invalid rows',preview.invalid],['Import readiness',preview.invalid?'Blocked until invalid rows are corrected.':'Subject to a fresh conflict check when applied.'],['Legend','Yellow cells show proposed changed values. Current values appear alongside them. Choices reflect the selections at download time.'],['Not an import template','This comparison workbook is for review. Edit the original Projects workbook to revise imported data.']]);
