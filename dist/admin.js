...
function updateDraftStatus(){const dirty=hasProjectDraft();$('#draft-status').textContent=(current?'Project '+String(numberedIds.indexOf(current.id)+1).padStart(2,'0'):'New project')+' · '+(dirty?'Unsaved changes':'No unsaved changes');$('#admin-cancel').disabled=busy||!dirty;form.querySelector('button[type=submit]').disabled=busy||!dirty;}
...
$('#admin-cancel').onclick=()=>{if(busy||!hasProjectDraft())return;discardDialog.returnValue='keep';discardDialog.showModal();discardDialog.querySelector('[value=keep]').focus();};
discardDialog.addEventListener('close',()=>{if(discardDialog.returnValue==='discard'){load(current,true);tell('Unsaved project changes discarded.');}$('#admin-cancel').focus();});
