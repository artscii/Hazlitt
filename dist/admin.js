...
function revealEditorBack(){
 let flip=false,duration=720;try{const saved=JSON.parse(sessionStorage.getItem('atlas-editor-flip')||'null');flip=!!saved;if(Number.isInteger(saved?.duration))duration=Math.max(300,Math.min(1600,saved.duration));sessionStorage.removeItem('atlas-editor-flip');}catch{}
