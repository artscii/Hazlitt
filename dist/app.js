...
function updateEvidencePanels(){
 ...
 evidenceLayoutFrame=requestAnimationFrame(()=>{
  const subsetSize=[...document.querySelectorAll('.evidence-scroll article.program')].filter(row=>!row.hidden).length;
  document.querySelectorAll('.project-subset-size').forEach(label=>label.textContent=subsetSize+' project'+(subsetSize===1?'':'s')+' in results');
