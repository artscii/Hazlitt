...
let markerProjectIds=null,semanticProjectIds=null,semanticContinent=null,semanticQuery="";
...
function matchingProjectIds(query){
 if(semanticProjectIds)return new Set([...semanticProjectIds].filter(id=>(!markerProjectIds||markerProjectIds.has(id))&&(!semanticContinent||AtlasSearch.continentsOf(programsById.get(id)).includes(semanticContinent))));
...
function filterProfiles(){
 ...
 document.querySelectorAll('.directory').forEach(section=>{const count=[...section.querySelectorAll('article.program')].filter(card=>!card.hidden).length;section.hidden=count===0;const heading=section.querySelector('.section-heading h2');if(heading){const label=count===programs.length&&count>0?`All ${count} ${count===1?'Project':'Projects'}`:`${count} ${count===1?'Project':'Projects'}`;if(heading.textContent!==label)heading.textContent=label;}});
 numberSelectedRows();
...
function showDetail(place){
 ...
 let summary;
 if(records.length===1){const p=records[0];summary=`<div class="detail-block">${badge(p)}<h3>${p.name}</h3><div class="project-evidence-status">${badge(p)}</div><p>${p.short}</p><p class="contact-note">${p.publicationYear?`Published ${p.publicationYear} · `:''}${p.evidenceBasis||'Evidence basis not yet classified'}</p><a href="#${p.id}">View project evidence ↓</a></div>`;}
 else{
...
document.addEventListener('click',e=>{
 ...
 if(link.getAttribute('data-profile')===id){
  ...
  if(semanticProjectIds){markerProjectIds=new Set([id]);syncSearchMap();}
...
function selectContinent(name){
 clearSharedProject({preserveSemantic:true});closeTip(true);
...
function revealMobileSearchResults(){
 if(!matchMedia('(max-width:740px)').matches||document.body.dataset.mobileView==='list')return;
 const input=document.querySelector('#project-search'),top=input.getBoundingClientRect().top;
 setMobileView('list');
 window.scrollBy(0,input.getBoundingClientRect().top-top);
}
document.querySelector('#project-search').addEventListener('input',revealMobileSearchResults);
document.querySelector('.mobile-search-shortcut').addEventListener('click',event=>{
 event.preventDefault();setMobileView('list');const input=document.querySelector('#project-search');input.focus({preventScroll:true});input.scrollIntoView({block:'center',behavior:'instant'});
});
...
function searchPlace(place){
 ...
 if(!query&&!sharedProjectId&&!semanticProjectIds)return {...place,ids:orderProjectIds(place.ids)};
...
function clearSharedProject({preserveSemantic=false}={}){
 ...
 if(!preserveSemantic){semanticProjectIds=null;semanticQuery='';}semanticContinent=null;
...
function openSharedProject(){
 ...
 semanticProjectIds=null;semanticContinent=null;markerProjectIds=null;chosenMarkerName=null;sharedProjectId=null;sharedNotice.hidden=!id;
