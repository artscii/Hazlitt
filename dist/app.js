// Atlas v1.3.8 — Bombo is the default selected project and initial map location.
...
// v1.3.8: filter visible profiles without rebuilding cards or moving keyboard focus.
...
// v1.3.8: mark matches in text nodes only, preserving links and profile markup.
...
// v1.3.8: open with Bombo selected, while preserving the shared coastal marker.
...
// v1.3.8: native search clearing and restored browser state also reset all results.
...
// v1.3.8: normal openings begin at the page top; explicit anchors keep their destination.
if(!location.hash){
...
// v1.3.8: keep search results, map coverage and the first result in sync.
...
// v1.3.8: stable offshore marker positions; selection never changes layout.
...
// v1.3.8: hover connectors use country centers, independent of fixed marker positions.
...
// v1.3.8: two gentle country pulses on explicit marker selection.
for(const marker of document.querySelectorAll('.marker')){
