// Atlas v2.1.1 — Muted catalog record numbers remain consistent across filtered and reordered lists.
...
for(const country of catalog.countries){for(const d of country.paths){const path=document.createElementNS(svgNS,'path');path.setAttribute('d',d);path.setAttribute('fill',covered.has(country.name)?'#b6d5d9':'#d5dfe5');path.setAttribute('stroke','#f8fbfd');path.setAttribute('stroke-width','.12');base.append(path);const outline=path.cloneNode();outline.dataset.country=country.name;outline.removeAttribute('fill');outline.removeAttribute('stroke');outline.removeAttribute('stroke-width');outlines.append(outline);}}
...
const renderProgram=p=>`<article class="program" id="${p.id}"><div><a class="back-to-map" href="#map-overview"><svg class="map-return-icon" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"><path d="m3 5 6-2 6 2 6-2v16l-6 2-6-2-6 2Z"/><path d="M9 3v16M15 5v16"/></svg>Back to map</a><span class="record-number" aria-label="Record ${recordNumbers.get(p.id)}">Record ${String(recordNumbers.get(p.id)).padStart(2,'0')}</span>${badge(p)}<h3>${p.name}</h3><p class="geo">${p.geo}</p><p class="geo">${p.date}</p><a class="share-project" href="${escapeHTML(projectURL(p.id))}" aria-label="Share project: ${p.name}">Share project</a><span class="share-feedback" role="status"></span></div><div><p class="label">REPORTED OUTCOMES</p><p>${p.outcome}</p>${links(p)}</div><div><p class="label">SPONSORS & PARTNERS</p><p>${p.partners}</p>${p.tel?`<a class="phone" href="tel:${p.tel}">${p.phone}</a>`:p.email?`<a class="phone" href="mailto:${p.email}">${p.email}</a>`:''}<p class="contact-note">${p.contact}</p>${p.contactSource?`<a href="${p.contactSource}" target="_blank" rel="noopener">Contact source ↗</a>`:''}</div></article>`;
