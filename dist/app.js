...
for(const country of catalog.countries){for(const d of country.paths){const path=document.createElementNS(svgNS,'path');path.setAttribute('d',d);path.setAttribute('fill',covered.has(country.name)?'#b6d5d9':'#d5dfe5');path.setAttribute('stroke','#f8fbfd');path.setAttribute('stroke-width','.12');if(covered.has(country.name)){const title=document.createElementNS(svgNS,'title');title.textContent=country.name;path.append(title);path.setAttribute('aria-label',country.name);}base.append(path);const outline=path.cloneNode();outline.dataset.country=country.name;outline.removeAttribute('fill');outline.removeAttribute('stroke');outline.removeAttribute('stroke-width');outlines.append(outline);}}
