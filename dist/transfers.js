...
window.AtlasTransfers={mount({before,api,getCatalog,hasDraft,reloadRecords,isBusy,setBusy}){
 ...
 function setWorking(value){working=value;setBusy(value);section.setAttribute('aria-busy',String(value));$('#export-workbook').disabled=value;$('#preview-workbook').disabled=value||!file.files.length;$('#test-workbook').disabled=value||!file.files.length;file.disabled=value;$('#cancel-import').disabled=value;section.querySelectorAll('[data-import-key]').forEach(control=>control.disabled=value);updateSelection();}
 ...
 async function previewWorkbook(testOnly){
  ...
  try{let result;try{result=await runWorkbook({action:'import',buffer:await chosen.arrayBuffer(),countries:getCatalog().countries});}catch(error){if(!testOnly)throw error;result={rows:[{row:0,name:'Workbook validation',error:error.message}]};}
   const data=await api('/api/imports/preview',{method:'POST',body:JSON.stringify({filename:chosen.name,rows:result.rows,testOnly})});if(generation!==fileGeneration)return;plan=data;renderPreview();
