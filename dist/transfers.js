...
window.AtlasTransfers={mount({before,api,getCatalog,hasDraft,reloadRecords,isBusy,setBusy}){
 ...
 function setWorking(value){working=value;setBusy(value);section.setAttribute('aria-busy',String(value));$('#export-workbook').disabled=value;$('#preview-workbook').disabled=value||!file.files.length;file.disabled=value;$('#cancel-import').disabled=value;section.querySelectorAll('[data-import-key]').forEach(control=>control.disabled=value);updateSelection();}
 ...
 file.onchange=()=>{fileGeneration++;plan=null;preview.hidden=true;message('');$('#preview-workbook').disabled=!file.files.length;};
 $('#preview-workbook').onclick=async()=>{
  ...
  try{const result=await runWorkbook({action:'import',buffer:await chosen.arrayBuffer(),countries:getCatalog().countries});
   const data=await api('/api/imports/preview',{method:'POST',body:JSON.stringify({filename:chosen.name,rows:result.rows})});if(generation!==fileGeneration)return;plan=data;renderPreview();
 ...
 function renderPreview(){
  preview.hidden=false;$('#import-summary').textContent=`${plan.total} rows: ${plan.added} new, ${plan.updates} changed matches, ${plan.skipped} duplicates, ${plan.invalid} need correction.`;
  ...
  for(const row of plan.rows){
   ...
   const title=document.createElement('h4');title.textContent='Row '+row.row+' · '+row.name;card.append(title);
 ...
 function updateSelection(){const selected=selection(),added=selected.filter(control=>control.dataset.action==='new').length,updated=selected.length-added;$('#import-selection').textContent=plan?`${added} to add · ${updated} to update · ${plan.total-selected.length} to skip`:'';$('#download-diff').disabled=working||!plan?.id;apply.disabled=working||!plan?.id||!selected.length||!!plan.invalid;}
 $('#download-diff').onclick=async()=>{
  if(working||isBusy()||!plan?.id)return;
  const selectedRows=selection().map(control=>Number(control.dataset.importKey));setWorking(true);message('Preparing the bulk import comparison…');
  try{const filename='Hazlitt-Bulk-Import-Diff-'+new Date().toISOString().slice(0,10)+'.xlsx';
   ...
   message('Diff preview download ready. Review the row decisions and highlighted field differences in Excel. No projects have changed.');await refreshHistory();
  }catch(error){message(error.message);}finally{setWorking(false);}
 };
 $('#cancel-import').onclick=()=>{plan=null;preview.hidden=true;message('Import preview cancelled. No records changed.');};
 apply.onclick=async()=>{
  if(working||isBusy()||!plan?.id)return;const keys=selection().map(control=>Number(control.dataset.importKey));
  if(!await confirmAction('Apply this import?',$('#import-selection').textContent+'. Changes will be visible to all visitors and can be undone from Global version history.'+(hasDraft()?' Your unsaved project draft will be discarded.':''),'Apply import'))return;
