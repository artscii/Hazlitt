...
Container recipe 1.0.0 packages the existing Atlas UI 1.1.1. The site is static:
...
```sh
docker build --pull -t hazlitt-creek-evidence-atlas:1.1.1 .
docker run -d --name hazlitt-atlas --restart unless-stopped \
  -p 127.0.0.1:8081:8080 hazlitt-creek-evidence-atlas:1.1.1
```
...
docker build --build-arg NGINX_IMAGE=nginxinc/nginx-unprivileged@sha256:YOUR_VERIFIED_DIGEST \
  -t hazlitt-creek-evidence-atlas:1.1.1 .
```
