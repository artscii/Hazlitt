...
for(const [name,title] of [['admin','Admin'],['log','Edit log']]){
 ...
 fs.writeFileSync(`dist/${name}.html`,`<!doctype html><html lang="en">${pageHead}<body>${header}<main class="management-page"><nav class="page-navigation" aria-label="Management navigation"><a href="/">Back to Atlas</a><a href="/admin" ${name==='admin'?'aria-current="page"':''}>Admin</a><a href="/log" ${name==='log'?'aria-current="page"':''}>Edit log</a></nav><h1>${title}</h1><div id="${name}-page" class="${name==='log'?'admin-section edit-log':''}"></div></main>${footer}</body></html>`);
}
const assets={};for(const name of ['index.html','admin.html','log.html','app.js','admin.js','log.js','style.css','map.svg'])assets['/'+name]={body:fs.readFileSync('dist/'+name,'utf8'),type:({'html':'text/html; charset=utf-8','js':'application/javascript; charset=utf-8','css':'text/css; charset=utf-8','svg':'image/svg+xml'})[name.split('.').pop()]};
