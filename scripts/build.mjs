...
for(const [name,title] of [['admin','Admin']]){
 ...
 const pageHead=head.replace('<head>','<head>'+flipBootstrap).replace('<title>Hazlitt Creek Evidence Atlas</title>',`<title>${title} · Hazlitt Creek Evidence Atlas</title>`).replace('</head>',`<script defer src="/transfers.js"></script><script defer src="/${name}.js"></script></head>`);
