...
function statement(sql,args=[]){return{bind(...next){return statement(sql,next)},async all(){const prepared=sqlite.prepare(sql);if(prepared.columns().length)return{results:prepared.all(...args)};const r=prepared.run(...args);return{results:[],meta:{changes:Number(r.changes)}}},async first(){return sqlite.prepare(sql).get(...args)||null},async run(){const r=sqlite.prepare(sql).run(...args);return{meta:{changes:Number(r.changes)}}}}};const DB={prepare:statement,async batch(statements){sqlite.exec('BEGIN');try{const out=[];for(const s of statements)out.push(await s.all());sqlite.exec('COMMIT');return out;}catch(e){sqlite.exec('ROLLBACK');throw e;}}};
const pilot=process.env.QMD_PILOT==='1'?await import('../pilot/service.mjs'):null;
const port=Number(process.env.PORT||8080);
http.createServer(async(req,res)=>{try{const chunks=[];let length=0;for await(const c of req){length+=c.length;if(length>(req.url==='/api/imports/preview'?8*1024*1024:65536)){res.writeHead(413);res.end();return;}chunks.push(c);}const request=new Request(requestOrigin(req.headers.host,process.env.PUBLIC_ORIGIN)+req.url,{method:req.method,headers:{...req.headers,'CF-Connecting-IP':req.socket.remoteAddress||'Unavailable'},body:['GET','HEAD'].includes(req.method)?undefined:Buffer.concat(chunks)});const env={DB,ADMIN_PASSWORD_HASH:process.env.ADMIN_PASSWORD_HASH};const response=pilot&&new URL(request.url).pathname.startsWith('/api/search-pilot/')?await pilot.pilotRoute(request,async()=>await (await worker.fetch(new Request('http://localhost/api/catalog'),env)).json()):await worker.fetch(request,env);res.writeHead(response.status,Object.fromEntries(response.headers));res.end(Buffer.from(await response.arrayBuffer()));}catch(e){console.error(e);res.writeHead(500);res.end('Unavailable');}}).listen(port,pilot?'127.0.0.1':'0.0.0.0',()=>console.log('Atlas running on port '+port));
