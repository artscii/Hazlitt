...
async function previewImport(env,input){
 ...
 const seenTargets=new Set(),seenValues=new Map(),seenInputIds=new Map(),seenInputNames=new Map(),rows=[],candidates=[];
 let added=0,updates=0,skipped=0,invalid=0;
 for(let index=0;index<input.rows.length;index++){
  const source=input.rows[index],row={key:index,row:Number.isInteger(source?.row)?source.row:index+2,name:String(source?.name||'Unnamed project').slice(0,200)};
  try{
   if(!source||typeof source!=='object'||Array.isArray(source))throw new Error('Invalid project row.');
   if(source.error)throw new Error(String(source.error).slice(0,300));
   const key=typeof source.name==='string'?nameKey(source.name):'',id=String(source.id||'').trim();
   if(id&&knownIds.has(id)&&!ids.has(id))throw new Error('This Project ID was deleted. Restore it through project history instead of importing it.');
   const byId=ids.get(id),byName=names.get(key);
   if(byId&&byName&&byId.id!==byName.id)throw new Error('Project ID and project name match different records. Correct this row before importing.');
   if(source.related!==undefined&&typeof source.related!=='boolean')throw new Error('Related initiative must be Yes or No.');
   const match=byId||byName,record=validate({...match,...source});if(!nameKey(record.name))throw new Error('Enter a recognizable project name.');
   const target=match?.id||(id&&seenInputIds.get(id))||seenInputNames.get(key)||nameKey(record.name);
   if(seenTargets.has(target)){
    if(seenValues.get(target)!==JSON.stringify(record))throw new Error('Conflicting rows in this file refer to the same project. Keep one intended row and preview again.');
    row.status='duplicate';row.message='Repeated within this spreadsheet; skipped.';skipped++;
   }else{
    seenTargets.add(target);seenValues.set(target,JSON.stringify(record));if(id)seenInputIds.set(id,target);seenInputNames.set(key,target);record.id=match?.id||'project-'+crypto.randomUUID();
    const comparable=(key,value)=>key==='countries'?[...(value||[])].sort():key==='related'?!!value:value??'';
 ...
 await database(env).prepare("INSERT INTO file_operations (id,kind,filename,at,status,payload,ip) VALUES (?,'import',?,?,'preview',?,?)").bind(id,transferFilename(input.filename),Date.now(),payload,transferIp(input.request)).run();
 return {id,rows,...summary};
}
async function applyImport(env,request,id){
 const operation=await transferRow(env,id);if(!operation||operation.kind!=='import')return json({error:'Import preview not found. Preview the file again.'},404);
 ...
 const chosen=payload.records.filter(item=>selected.has(item.key)),current=await records(env);
 for(const item of chosen){
  const live=current.find(p=>p.id===item.record.id),duplicate=current.find(p=>nameKey(p.name)===item.nameKey&&p.id!==item.record.id);
  if(duplicate||(item.before&&(!live||live.revision!==item.baseRevision)))return json({error:'A project changed since this preview. Preview the file again before applying changes.'},409);
 }
 payload={...payload,records:chosen,added:chosen.filter(item=>!item.before).length,updated:chosen.filter(item=>item.before).length,skipped:payload.total-chosen.length};
 const serialized=JSON.stringify(payload);
 const db=database(env),at=Date.now();
 try{
  ...
  const fresh=await transferRow(env,id);if(fresh?.status==='applied')return json({ok:true,id,...JSON.parse(fresh.payload),records:undefined});
  return json({error:'Nothing was imported. A record conflict or storage error occurred. Preview the file again.'},409);
 }
 return json({ok:true,id,added:payload.added,updated:payload.updated,skipped:payload.skipped});
}
async function undoImport(env,request,id){
 const operation=await transferRow(env,id);if(!operation||operation.kind!=='import')return json({error:'Import not found.'},404);
 if(operation.status==='undone')return json({ok:true,alreadyUndone:true});
 if(operation.status!=='applied')return json({error:'Only a completed import can be undone.'},409);
 const payload=JSON.parse(operation.payload),db=database(env),at=Date.now();
...
async function transferRoute(request,env,path,url){
 if(path==='/api/imports/preview'&&request.method==='POST'){const input=await body(request,8*1024*1024);return json(await previewImport(env,{...input,request}));}
 const commit=path.match(/^\/api\/imports\/([a-f0-9-]+)\/commit$/);if(commit&&request.method==='POST')return applyImport(env,request,commit[1]);
 const undo=path.match(/^\/api\/imports\/([a-f0-9-]+)\/undo$/);if(undo&&request.method==='POST')return undoImport(env,request,undo[1]);
 if(path==='/api/exports'&&request.method==='POST'){
  const input=await body(request),programs=await records(env),id=crypto.randomUUID(),filename=transferFilename(input.filename);
  let details={count:programs.length},preview;
  if(input.kind==='diff'){
   const source=await transferRow(env,input.previewId);
   if(!source||source.kind!=='import'||source.status!=='preview'||Date.now()-source.at>1800000)return json({error:'Preview the spreadsheet again before exporting its comparison.'},409);
 ...
 if(path==='/api/global-history'&&request.method==='GET'){
  ...
  const entries=(await database(env).prepare("SELECT g.*,f.status AS operation_status,CASE WHEN f.kind='import' AND f.status='applied' AND NOT EXISTS(SELECT 1 FROM json_each(f.payload,'$.records') j LEFT JOIN records r ON r.id=json_extract(j.value,'$.record.id') WHERE r.id IS NULL OR r.deleted<>0 OR r.revision<>json_extract(j.value,'$.importRevision')) THEN 1 ELSE 0 END AS can_undo FROM global_history g JOIN file_operations f ON f.id=g.operation_id WHERE g.version<? ORDER BY g.version DESC LIMIT 21").bind(before).all()).results;
  return json({entries:entries.slice(0,20).map(row=>({...row,details:JSON.parse(row.details),can_undo:!!row.can_undo})),hasMore:entries.length>20});
 }
 return null;
}
